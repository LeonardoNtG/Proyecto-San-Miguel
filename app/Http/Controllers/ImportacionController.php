...
class ImportacionController extends Controller
{
    ...
    public function mantenimientoBloquesCampana(Request $request)
    {
        ...
        $bloqueEliminar = $request->get('eliminar');
        $eliminadosInfo = [];

        if (!empty($bloqueEliminar)) {
            DB::beginTransaction();
            try {
                $bloquesMatch = Bloque::withoutGlobalScope('lotificacion')
                    ->where('lotificacion_id', $campana->id)
                    ->where(function($q) use ($bloqueEliminar) {
                        $q->where('nombre', $bloqueEliminar)
                          ->orWhere('nombre', 'like', "%{$bloqueEliminar}%");
                    })
                    ->get();

                foreach ($bloquesMatch as $b) {
                    $lotes = Lote::withoutGlobalScope('lotificacion')->where('id_bloque', $b->id_bloque)->get();
                    $loteIds = $lotes->pluck('id_lote')->toArray();

                    HistorialLote::whereIn('id_lote', $loteIds)->delete();
                    $lotesCount = Lote::withoutGlobalScope('lotificacion')->where('id_bloque', $b->id_bloque)->delete();
                    $b->delete();

                    $eliminadosInfo[] = [
                        'id_bloque' => $b->id_bloque,
                        'nombre'    => $b->nombre,
                        'lotes_eliminados' => $lotesCount
                    ];
                }
        ...
        // Listado actualizado de bloques en La Campana
        $bloquesCampana = Bloque::withoutGlobalScope('lotificacion')
            ->where('lotificacion_id', $campana->id)
            ->get()
            ->map(function($b) {
                return [
                    'id_bloque' => $b->id_bloque,
                    'nombre'    => $b->nombre,
                    'lotes'     => Lote::withoutGlobalScope('lotificacion')->where('id_bloque', $b->id_bloque)->count()
                ];
            });

        return response()->json([
            'proyecto' => $campana->nombre,
            'lotificacion_id' => $campana->id,
            'eliminados' => $eliminadosInfo,
            'bloques_actuales' => $bloquesCampana,
        ]);
    }
